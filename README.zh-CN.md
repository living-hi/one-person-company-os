...
**One Person Company OS 是一套面向 AI Native Solo Founder 的快循环公司运行系统。**
...
One Person Company OS 的目标不是再多给一点建议，而是提供一层真正的 **Company Operating Layer**，让一人公司按 AI 时代的快循环运行。
...
更合理的默认流程，不应该是先做一套繁重阶段管理，再慢慢补动作。

更匹配 AI 时代的一条主循环是：

- 先用一句话收窄用户和痛点
- 快速调研市场与用户需求，拿到真实证据
- 产出最小可上线 MVP，而不是大而全版本
- 小范围上线，优先跑通反馈闭环
- 根据使用、反馈、故障和转化持续改进
- 价值、留存和交付稳定后，再放大推广和增长

这个项目现在把上面这条快循环当成用户默认玩法。
内部保留 `验证期 / 构建期 / 上线期 / 运营期 / 增长期`，但它们只用于标记当前主瓶颈，不应该被用户感知成重型官僚流程。
...
注意：这是“单次执行协议”，不是要求创始人按重型生命周期思考公司。
对用户来说，最重要的仍然是前面的 AI 快循环。
...
- 如果进入上线期、运营期或增长期，会继续自动补齐 `产物/04-部署与生产/` 和 `产物/05-上线与增长/` 下的对应 starter docs

这些 starter 文件会先以 `[待生成]` 状态落盘，让你先看清完整交付版图；正式补齐后会切换成 `[已生成]`。
...
```text
我正在创立一间 AI 一人公司，请帮我调用 one-person-company-os。先别让我填很多表，先主动问我一句创业想法；如果我没想好，就给我 3 到 4 个方向建议让我选。默认按 AI 时代的一人公司快循环来：快速验证需求，快速做 MVP，小范围上线，收反馈再迭代。先做 Step 1/5 到 Step 3/5，给我公司创建草案，并告诉我当前内容有没有保存、之后去哪个路径查看。
```
...
先别让我填很多字段，先主动问我一句创业想法；如果我还没想好，就给我 3 到 4 个方向建议。
默认按 AI 时代的一人公司快循环来：快速验证需求，快速做 MVP，小范围上线，收反馈再迭代。
先做 Step 1/5 到 Step 3/5，给我公司创建草案，并告诉我内容是否已保存、我接下来该看哪个路径；
我确认后再正式创建。
```
