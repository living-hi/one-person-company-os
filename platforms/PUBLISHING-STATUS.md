...
| MCP Registry | published | 1.0.3 | `platforms/mcp-server/`, `ghcr.io/living-hi/one-person-company-os-mcp:1.0.3` | Official MCP Registry API returns `io.github.living-hi/one-person-company-os`; `publishedAt=2026-05-06T02:52:00Z`; GitHub Actions run `25413862092` succeeded | none | monitor downstream registry sync and publish updated versions through the GitHub Actions OIDC workflow |
| Smithery | ready-to-submit | 1.0.3 | `platforms/mcp-server/`, `platforms/mcp-registries/smithery.md` | Smithery submission note generated; Official MCP Registry package now exists | requires Smithery account/API key plus either an MCPB bundle or public HTTPS MCP endpoint for direct publish | authenticate with Smithery and publish a URL or MCPB package, then record listing URL |
| Glama | ready-to-submit | 1.0.3 | `platforms/mcp-server/`, `platforms/mcp-registries/glama.md` | Glama says it is a superset of the Official MCP Registry; official registry entry now exists | Glama indexing has not yet shown a visible listing for `one-person-company-os` | re-check Glama indexing after registry propagation or request indexing from the account flow |
| PulseMCP | ready-to-submit | 1.0.3 | `platforms/mcp-server/`, `platforms/mcp-registries/pulsemcp.md` | PulseMCP submission note generated; official registry entry now exists | PulseMCP search/submission still requires public listing evidence or account/manual flow | wait for downstream sync or submit registry metadata manually and record listing URL |
